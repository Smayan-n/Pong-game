...
def mouseAsk():
	global mouseInput
	mouse = pyautogui.confirm(text='Select your game controls', title='Selcet Game Options', buttons=['Arrow Keys', 'Mouse'])
...
def gameDifficulty():
	global infinite
	game_option = pyautogui.confirm(text='Select your game difficulty', title='Selcet Game Mode', buttons=['Beginner', 'Intermediate', 'Advanced', 'INFINITE'])

	if game_option == 'Beginner':
		i = 0
		while i != 5:
			comp_rand.append(i)
			i += 1
	elif game_option == 'Intermediate':
		i = 0
		while i != 11:
			comp_rand.append(i)
			i += 1
	elif game_option == 'Advanced':
		i = 0
		while i != 26:
			comp_rand.append(i)
			i += 1

	elif game_option == 'INFINITE':
		infinite = True
...
class computer():
	...
	def __init__(self):
		self.rand_num = -1
		self.rand_event = False
		self.bouncer_move = True
	...
	def path(self):
		...
		if ball.start:
			...
			if infinite == False:
				if ball.x < 145 and ball.left:
					self.rand_event = True
				if ball.x < 141 and ball.left:
					self.rand_event = False

				if self.rand_event:
					self.rand_num = random.choice(comp_rand)
					if self.rand_num == 0:
						i = 0
						while i < 100:
							self.bouncer_move = False
							i += 1
						self.rand_num = -1


			if self.bouncer_move:
				bouncer1.y = ball.y - bouncer1.height//2

			if bouncer1.y < 1:
				bouncer1.y = 1
			if bouncer1.y > win_height - bouncer1.height - 1:
				bouncer1.y = win_height - bouncer1.height - 1
...
class ball():
	...
	def collision(self):
		...
		if self.y >= bouncer1.y and self.y <= bouncer1.y + bouncer1.height//3:
			if self.x >= bouncer1.x and self.x <= bouncer1.x + bouncer1.width:
				self.y_vel = random.choice(y_vels)
		
		if self.y >= bouncer1.y + 2*(bouncer1.height//3) and self.y <= bouncer1.y + bouncer1.height:
			if self.x >= bouncer1.x and self.x <= bouncer1.x + bouncer1.width:
				self.y_vel = random.choice(y_vels) * -1

		
		if self.y >= bouncer2.y and self.y <= bouncer2.y + bouncer2.height//3:
			if self.x >= bouncer2.x and self.x <= bouncer2.x + bouncer2.width:
				self.y_vel = random.choice(y_vels) * -1
		
		if self.y >= bouncer2.y + 2*(bouncer2.height//3) and self.y <= bouncer2.y + bouncer2.height:
			if self.x >= bouncer2.x and self.x <= bouncer2.x + bouncer2.width:
				self.y_vel = random.choice(y_vels)
		...
		if self.x < 0:
			...
			player1_point_lost = font2.render('Player = Score +1' ,1, (255, 0, 0))
		...
		if self.x > win_width:
			...
			player2_point_lost = font2.render('computer = Score +1' ,1, (255, 0, 0))
		...
		# seeing if ball hits edges asnd change direction
...
def redrawGameWin():
	...
	player1_text = font3.render('Computer', 1, (0, 255, 0))
	player2_text = font3.render('Player', 1, (0, 255, 0))
...
while run:
	...
	if mouseInput:
		#if ball.start:
			if bouncer2.y > 0 and bouncer2.y + bouncer2.height < win_height:
				bouncer2.y = my - (bouncer2.height // 2)
								
				if bouncer2.y < 1:
					bouncer2.y = 1
				if bouncer2.y > win_height - bouncer2.height  - 1:
					bouncer2.y = win_height - bouncer2.height - 1
